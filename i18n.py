...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "zh": {
        ...
        "⚠️ `skopeo` command not found; tasks will fail immediately on submit. macOS: %(install)s.": "⚠️ 未检测到 skopeo 命令，任务提交后会立即失败。macOS 安装：%(install)s。",
        ...
        "`docker`: full path like `docker.io/(library/|project/)image:tag`;": "docker： 完整路径如 docker.io/(library/|project/)image:tag；",
        "`docker-daemon`: only the local image:tag, e.g. `(library/|project/)image:tag`.": "docker-daemon： 只需填本地的image:tag，如 (library/|project/)image:tag。",
        ...
        "Skopeo options": "Skopeo 选项",
        "Multi-arch": "多架构",
        "Retry times": "重试次数",
        "Push the full multi-arch manifest list (amd64 / arm64 / armv7).": "推送整张多架构 manifest list（amd64 / arm64 / armv7 等）。",
        "Skopeo retry count on transient errors. 0 = no retries; max 10; default 3.": "skopeo 在瞬时错误时的重试次数。0 = 不重试；最大 10；默认 3。",
        "push the full manifest list (amd64 / arm64 / armv7, etc). Recommended for multi-arch images like nginx from an ARM Mac.": "推送整张 manifest list（amd64 / arm64 / armv7 等）。多架构镜像（如 Mac ARM 上的 nginx）建议勾选。",
        "how many times skopeo retries on transient errors (network blip, registry 5xx). 0 = no retries; max 10. Default 3 matches skopeo's own default.": "skopeo 在瞬时错误（网络抖动、registry 5xx 等）时的重试次数。0 = 不重试；最大 10；默认 3 与 skopeo 自身默认值一致。",
        "Start Copy": "开始拷贝",
        "Recent tasks": "最近任务",
        "Status": "状态",
        ...
        "Unsupported source type: %(t)s": "不支持的源类型：%(t)s",
        ...
        "skopeo command not found. Install first: brew install skopeo": "skopeo 命令未找到。请先安装：brew install skopeo",
        "Failed to start skopeo: %(err)s": "启动 skopeo 失败：%(err)s",
        "skopeo exited with code %(rc)s": "skopeo 退出码 %(rc)s",
        ...
        "Task had no heartbeat for over %(sec)ss; worker/skopeo may have hung, auto-marked as failed": "任务超过 %(sec)ss 未上报心跳，worker/skopeo 可能已挂起，已自动标记为失败",
        ...
        "Cannot parse skopeo output: %(err)s": "无法解析 skopeo 输出：%(err)s",
